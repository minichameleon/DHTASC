...
def getDistance(lat1, lng1, lat2, lng2):
    ...
    radLat1 = rad( lat1 ) # 将数值转换为弧度值
    radLat2 = rad( lat2 ) # 将数值转换为弧度值
    a = radLat1 - radLat2
    b = rad( lng1 ) - rad( lng2 )
...
def plan_of_exec_sequence(p_profile,Tid,allTask):
    ...
    def relax_degree_calcute(now_ponit,res_point,point_info,now_time,distance):
        ...
        relax_temp = [0 for i in range(len(res_point))]  # 松弛度缓存区
        # now_time = int(point_info[now_ponit][2])
        for ri in range(len(res_point)):
            task_deadline = int(point_info[res_point[ri]][2]) # 读取任务截止时间
            # 因为使用的数据集是采用出租车接送乘客的位置信息，因此任务点之间的距离可能十分大，正常人的移动短时间内无法到达，因此在模拟仿真的过程中将彼此之间的距离缩短10倍
            move_time = distance[res_point[ri]]*1000/v # 计算参与者的移动时间
            relax_temp[ri] = task_deadline - move_time - now_time # 计算松弛度
        return relax_temp
...
def change_profile(replaceid,raw_profile,refer_profile):
    ...
    :return: 替换后的参与者概貌
    """
    # 复制原参与者概貌信息，列表复制需要使用copy  复制符号无法达到效果
...
def allocate_bizhi(filename,filename2):
    # 载入数据
    ...
    # 任务完成率为：TFR,移动距离为：TPD,综合效用为：TPU
    TFR = 0
    TPD = 0
    TPU = 0
    # 已经完成的任务数量
    ...
    while len(resTASK)>0:
        ...
        participant_profile_change_refer = [] # 参与者概貌改变的参考
        global_finish_rate = [] # 全局任务完成率
        global_distance = []    # 全局移动距离
        global_task_loss_rate= [] # 全局任务损失率
        utility= []
        avg_global_task_loss_rate = [] # 平均任务损失率
        # 尝试的任务顺序
        ...
        for i in range(TaskLen):
            ...
            # 如果这个任务所在的类中任务数量只有一项
                ...
                need_sensor = set(Task[str(taskid[0])][1]) # 需要的传感器
                budget = int(Task[str(taskid[0])][2])   # 预算
                ...
                for pid in range( participantLen ):
                    ...
                    can_exec_s = need_sensor.issubset( sensor_p )  # 在类型上判断能否执行
                    paypal_temp = 0
                    can_exec_n = verify_execution_capability(worker[1],need_sensor_num)  # 验证执行能力
                    if can_exec_s and can_exec_n:  # 如果传感器类型满足 且执行能力也满足
                        taskt = Task[str( taskid[0] )][1] # 获取任务所需的传感器
                        for taskt_sensor in taskt:
                            for p_sensor_id in range( len( worker[1] ) ):
                                if taskt_sensor == worker[1][p_sensor_id][0]: # 判断任务所需的传感器和参与者的能否匹配
                                    if worker[1][p_sensor_id][1] != 0: #负载是否满足要求
                                        paypal_temp = paypal_temp + worker[1][p_sensor_id][1]
                                    else:
                                        paypal_temp = paypal_temp + 1
                                    worker[1][p_sensor_id][2] = worker[1][p_sensor_id][2] - 1
                ...
                for selectid in idx:
                    ...
                    if count>=need_participant_max:
                        break
                #计算任务的完成率
                if count<need_participant_min: # 招募到的参与者人数小于最小值时
                    f_i = 0
                    participant_profile_change_refer.append( participant_profile_temp_2 )
                    global_distance.append(sys.maxsize)
                elif count>=need_participant_min and count <need_participant_max: # 招募到的参与者人数在最小值和最大值之间
                    f_i = count
                ...
                if need_participant_max == need_participant_min: # 如果任务所需的参与者人数最小值与最大值相同
        ...
        bizhi = [0 for bz in range(len(global_distance))]
        # distance中除了0之外的最小值,
        min_addition_zero = min([mind for mind in global_distance if  mind !=0])
        # 如果distance 等于0，将其表示为最小值的1/10
        zhuanhuanzero = min_addition_zero*1.0/10
        for bz in range(len(global_distance)):
            if global_distance[bz]==0:
                bizhi[bz] = global_finish_rate[bz] *1.0  /  zhuanhuanzero
            else:
                bizhi[bz] = global_finish_rate[bz] /global_distance[bz]
        # sorted_utility = sorted( enumerate( utility ), key=lambda x: x[1] )  # 将 paypal从小到大排序
        sorted_bizhi = sorted( enumerate( bizhi ), key=lambda x: x[1] ,reverse=True)
        idxu = [i[0] for i in sorted_bizhi]
...
if __name__ == "__main__":
    print( 6502 )
    filename = 'newdata_d100data_start_with_sensor/6510.json'
    filename2 = 'newdata_d100data_end_with_task/6510.txt'
    tfr,tpd,tpu,aclocated_task_num = allocate_bizhi(filename,filename2)
    print("-"*20)
